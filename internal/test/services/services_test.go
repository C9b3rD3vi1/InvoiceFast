...
func TestClientCRUD(t *testing.T) {
	...
	client, err := clientService.CreateClient(user.TenantID, user.ID, &clientReq)
	require.NoError(t, err)
	assert.NotEmpty(t, client.ID)
	assert.Equal(t, "Test Client", client.Name)

	// Test GetClient
	fetchedClient, err := clientService.GetClient(user.TenantID, client.ID)
	require.NoError(t, err)
	...
	updatedClient, err := clientService.UpdateClient(user.TenantID, client.ID, &updateReq)
	require.NoError(t, err)
	assert.Equal(t, "Updated Client", updatedClient.Name)

	// Test GetUserClients
	clients, total, err := clientService.GetUserClients(user.TenantID, ClientFilter{})
	require.NoError(t, err)
	assert.Greater(t, total, int64(0))
	assert.Greater(t, len(clients), 0)

	// Test DeleteClient
	err = clientService.DeleteClient(user.TenantID, client.ID)
	require.NoError(t, err)
}
...
func TestClientValidation(t *testing.T) {
	...
	_, err := clientService.CreateClient(user.TenantID, user.ID, &req)
	assert.Error(t, err)
}
...
func TestClientSearch(t *testing.T) {
	...
	// Create clients
	clientService.CreateClient(user.TenantID, user.ID, &CreateClientRequest{
	...
	// Search
	clients, _, err := clientService.GetUserClients(user.TenantID, ClientFilter{
		Search: "searchable",
	})
	require.NoError(t, err)
...
func TestInvoiceCRUD(t *testing.T) {
	user := createTestUser(t)
	client := createTestClient(t, user.TenantID, user.ID)
...
func createTestClient(t *testing.T, tenantID, userID string) *models.Client {
	t.Helper()
	...
	client, err := clientService.CreateClient(tenantID, userID, &req)
	require.NoError(t, err)
	return client
}
